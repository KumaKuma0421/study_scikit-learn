...
def estimator_response_save(
    ...
    if hasattr(estimator, "decision_function"):
        z_value = estimator.decision_function(
            np.c_[xx_val.ravel(), yy_val.ravel()])
    else:
        z_value = estimator.predict_proba(
            np.c_[xx_val.ravel(), yy_val.ravel()])[:, 1]

    z_value = z_value.reshape(xx_val.shape)

    data = pd.DataFrame(
        columns=["row", "col", "x_value", "y_value", "z_value"])
    data["x_value"] = xx_val.ravel()
    data["y_value"] = yy_val.ravel()
    data["z_value"] = z_value.ravel()
