...
def abstract_data(target_file):
    ...
    group_data_1 = data.groupby(by=["estimator", "validator", "param", "point"])
        ...
        for validator in common.VALIDATORS:
            for param in target_params:
                for num in target_count:
                    row = f"{estimator}"
                    col = f"accuracy({validator}-{num}-p{param})"
                    mean = group_data_1.get_group((estimator, validator, param, num))[
                        "accuracy"].mean()
