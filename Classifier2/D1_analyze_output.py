...
def analyze_data(file_path):
    ...
    for idx, group in data_by_pattern:
        ...
        pivot[f"mean_fit_time_{idx}"] = values["mean_fit_time"]
    ...
    save_file = common.make_path(
        common.DATA_PATH, common.REPORT_SUB_PATH,
        "report_1_mean_fit_time.csv")
...
def plot_data(data):
    ...
    for i in np.arange(len(label)):
        if i < 5:
            axes[1].plot(mean_fit_time_label.values(),
                         data.iloc[i, 1:-3].values, label=label[i])
        else:
            axes[1].plot(mean_fit_time_label.values(),
                         data.iloc[i, 1:-3].values, ls="--")
